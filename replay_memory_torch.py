    ...
    def getMinibatch(self):
        ...
        while len(indices) < self.batch_size:
            ...
            while True:
                ...
                if(self.count-1<self.hist_len+1):
                    print("<<<<<<")
                index = np.random.randint(self.hist_len+1, self.count-1)
            ...
            for i in range(1, self.hist_len + 1):
                ...
                pre_states_alpha[len(indices)][cur_ind] = self.states_alpha[index - i].clone()
                post_states_alpha[len(indices)][cur_ind] = self.states_alpha[index - i + 1].clone()
            indices.append(index)

        # copy actions, rewards and terminals with direct slicing
        actions = self.actions[indices].clone()
        rewards = self.rewards[indices].clone()
        terminals = self.terminals[indices].clone()
